<x-filament::page>
	@php
		$calendar = \App\Filament\Pages\Calendar::getDemoData();
		$badgeColors = [
			'О' => 'success',
			'П' => 'warning',
			'Р' => 'danger',
	...
	@foreach($calendar['months'] as $mi => $month)
		<div class="mb-8">
			<div class="font-semibold text-base mb-2 text-blue-800 dark:text-blue-300">{{ $month['name'] }} {{ $month['year'] }}</div>
			<div class="overflow-x-auto">
				<table class="min-w-full border rounded-xl text-center bg-white dark:bg-gray-900 shadow text-xs">
					...
					<tbody>
						@foreach($calendar['machines'] as $machine)
							<tr>
								<td class="font-medium border px-2 py-1 sticky left-0 bg-white dark:bg-gray-900 z-10 text-left" style="min-width:160px;">{{ $machine }}</td>
								@foreach($month['days'] as $day)
									@php
										$status = $calendar['data'][$machine][$mi][$day];
								...
								<td class="border px-1 py-1 bg-blue-50 sticky right-0 z-10">
									@foreach($badgeColors as $abbr => $color)
										<x-filament::badge :color="$color" size="sm" :tooltip="$badgeLabels[$abbr]">
											{{ $abbr }}: {{ $calendar['rowSums'][$machine][$mi][$abbr] ?? 0 }}
										</x-filament::badge>
									@endforeach
								</td>
							</tr>
					...
					{{-- Убрано полностью: <tfoot> Σ по дням --}}
				</table>
			</div>
		</div>
	@endforeach
